...
class MyApp(QDialog):
    ...
    def initUI(self):
        ...
        # 풀버젼
        # pyinstaller --hidden-import PyQt5 --hidden-import pyserial --hidden-import requests --hidden-import chardet --add-data="C:\\my_games\\hobbyexcel\\data_excel;./data_data_excel" --add-data="C:\\my_games\\hobbyexcel\\mysettings;./mysettings" --name hobbyexcel -i="ex_ccocco.ico" --add-data="ex_ccocco.ico;./" --icon="ex_ccocco.ico" --paths "C:\Users\1_S_3\AppData\Local\Programs\Python\Python311\Lib\site-packages\cv2" main.py
        # 업데이트버젼
...
class FirstTab(QWidget):
    ...
    def initUI(self):
        ...
        # 테스트 버튼
        self.mytestin = QPushButton('네이버')
        self.mytestin.clicked.connect(self.my_naver)
        self.perfect_pause = QPushButton('쿠팡')
        self.perfect_pause.clicked.connect(self.my_coopang)
    ...
    def my_coopang(self):
        ...
        try:
            ...
            if file_path:
                ...
                self.df_list.rename(
                    columns={a: aa, c: cc, d: dd, e: ee, f: ff, h: hh, g: gg, j: jj, k: kk, l: ll, m: mm, n: nn, is_option: option}, inplace = True)
                print("재정비")
                ...
                print("원하는 열 나열하기1")
                columns_list = self.df_list[[aa, cc, dd, ee, ff, gg, hh, jj, kk, ll, mm, nn, oo, pp, gagin, option]].values.tolist()
                ...
                for set_2 in range(result_set_2_len):
                    ...
                    for set_index in range(len(result.index)):
                        ...
                        if set_index == 0:
                            ...
                            if pd.isnull(df.loc[result.index[set_index], gg]) == True:
                            # if len(df.loc[result.index[set_index], gg]) == 0:
                                df.loc[result.index[set_index], dd] = result_write_num + df.loc[result.index[set_index], option] + ":" + df.loc[result.index[set_index], dd] + str(add_write)
